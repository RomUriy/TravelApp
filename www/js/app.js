...
.controller('ToDoCtrl', function( $scope, $ionicModal, $timeout ){
  ...
  $ionicModal.fromTemplateUrl('views/WindowCamera.html', function(WindowCamera){
    $scope.taskWindowCamera = WindowCamera;
  },{
    scope: $scope,
  ...
  $scope.WindowCamera = function() {
    $scope.activeTask = {
      email: ''
    }
    $scope.taskWindowCamera.show();
    $scope.currentTaskId = -1;
  }

  $scope.closeWindowCamera = function() {
    $scope.taskWindowCamera.hide();
  }
